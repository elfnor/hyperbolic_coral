...
"""
Updating for use as a Sverchok Scripted Node light
...
def reflect_face(
    edge_ind, face_ind, verts_out, edges_out, faces_out, edge_face_map, verts_rnd
):
    """
...
def poincare_tiling(p, q, nlayers):
    ...
    verts_rnd = {
        (round(v[0], ndigits), round(v[1], ndigits), round(v[2], ndigits)): i
    ...
    while len(faces_out) < max_faces:
        ...
        verts_new, edges_new, face_new, edge_face_map = reflect_face(
            edge_free, face, verts_out, edges_out, faces_out, edge_face_map, verts_rnd
        )
        n = len(verts_out)
        vnr = {
            (round(v[0], ndigits), round(v[1], ndigits), round(v[2], ndigits)): n + i
...
v1, e1, f1 = poincare_tiling(p, q, layers)
v2, e2, f2 = cone_tris(p, v1, e1, f1)
